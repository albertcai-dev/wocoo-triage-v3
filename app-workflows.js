...
function TriageWorkflow(props){var ticket=props.ticket,onClose=props.onClose;
  ...
  var isCCOvpType=(ticket.type||"").toLowerCase().indexOf("credit card: overpayment")!==-1;var approver=manualApprover||((amount&&amount>=5000)||new Date()<LUKE_OOO_UNTIL?APPROVERS.amanda:APPROVERS.luke);var meetsMinimum=amount&&amount>=1000;var overFraudThreshold=!!(amount&&amount>FRAUD_AMOUNT_THRESHOLD);
  ...
  // Ask the bridge whether a FRAUD ticket is already linked to this WOCOO ticket.
  // This cannot be done from the site directly: MCPLocker's jira_get_ticket returns
  // no link data at all (no linked_issues even with include_relations), and JQL
  // linkedIssues() comes back empty through MCPLocker. Jira itself is fine — FRAUD
  // is only limited in MCPLocker's tier system — so the bridge reads it under the
  // operator's own credentials. Only called for amounts over the threshold, since
  // that is the only case where the answer changes the route.
  var checkFraudLinks=useCallback(function(){
    setLinkCheck("checking");setLinkCheckError(null);
    return callBridgeViaIframe("getIssueLinks",{ticketId:ticket.id,projectFilter:FRAUD_PROJECT_KEY},"issueLinksLoaded").then(function(data){
      setLinkedFraudKeys((data&&data.matchedKeys)||[]);
      setLinkCheck("done");
    }).catch(function(e){
      ...
      setLinkedFraudKeys([]);
      setLinkCheck("failed");
      setLinkCheckError(e&&e.message?e.message:String(e));
    });
  },[ticket]);

  // Runs once the amount is known — after the JIRA fetch, or after a manual
  // amount override pushes the ticket over the threshold.
  useEffect(function(){
    if(step<1)return;
    if(linkCheck!=="idle")return;
    if(!(amount&&amount>FRAUD_AMOUNT_THRESHOLD))return;
    checkFraudLinks();
  },[step,amount,linkCheck,checkFraudLinks]);

  var fetchTicket=useCallback(function(){
    setLoading(true);setError(null);
    var fieldsSpec="summary,description,issuetype,status,priority,labels,created,reporter,customfield_11458,customfield_11416,customfield_24151,customfield_10082,customfield_24357";
